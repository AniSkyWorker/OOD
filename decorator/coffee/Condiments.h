...
{
public:
	CLiquor(IBeveragePtr && beverage, LiqorType const& liquorType)
		: CCondimentDecorator(move(beverage))
		, m_liqorType(liquorType)
	{}
protected:
	double GetCondimentCost()const override
	{
	...
	std::string GetCondimentDescription()const override
	{
		return std::string(m_liqorType == LiqorType::Chocolate ? "Chocolate" : "Walnut")
			+ " liquor";
	}

private:
	LiqorType m_liqorType;
};
