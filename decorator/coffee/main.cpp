...
{
	{
		...
		auto latte = make_unique<CoffeeType::CLatte>();
		// добавляем корицы
				...
				make_unique<CIceCubes>(				// | под нею - кубики льда
					make_unique<CLemon>(			// | | еще ниже лимон
						make_unique<CCinnamon>(		// | | | слоем ниже - корица
							make_unique<CoffeeType::CLatte>()),	// | | |   в самом сердце - Латте
						2),							// | | 2 дольки лимона
	...
	{
		auto beverage = 
			make_unique<CoffeeType::CLatte>()							// Наливаем чашечку латте,
	...
	{
		auto beverage = make_unique<CoffeeType::CLatte>(CoffeePortions::Double);

		cout << beverage->GetDescription() << " costs " << beverage->GetCost() << endl;
	}

	{
		auto beverage = make_unique<CoffeeType::CCapuccino>(CoffeePortions::Double);

		cout << beverage->GetDescription() << " costs " << beverage->GetCost() << endl;
	}

	{
		auto beverage = make_unique<CMilkshake>(MilkshakePortions::Large) << MakeCondiment<CCream>() << MakeCondiment<CLiquor>(LiqorType::Walnut);

		cout << beverage->GetDescription() << " costs " << beverage->GetCost() << endl;
	}

	{
		auto beverage = make_unique<TeaType::СRedTea>() << MakeCondiment<CChocolate>(13);
